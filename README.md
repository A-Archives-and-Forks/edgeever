...
- **Bring Your Own AI Models**: Connect OpenAI, Anthropic, or Gemini-compatible services and third-party API relays to empower your editor with smart note summarization, key point extraction, proofreading, translation, and text continuation on full notes or selected text. Desktop and mobile apps call your model API directly. The web app does the same when the model host allows CORS; otherwise it uses the instance proxy.
